...
export async function POST(req: NextRequest) {
    ...
    const imageContent = image ? Buffer.from(await image.arrayBuffer()).toString('base64') : '';

    const csvResponse = await fetch('https://cdn.jsdelivr.net/gh/alexandrenf/dataifmsabrazil/noticias.csv');
    ...
    try {
        ...
        if (!markdownResponse.ok) {
            ...
            throw new Error(`GitHub API responded with status ${markdownResponse.status}`);
        }
        ...
        if (imageContent) {
            ...
            if (!imageResponse.ok) {
                ...
                throw new Error(`GitHub API responded with status ${imageResponse.status}`);
            }
        }
        ...
        if (!csvUpdateResponse.ok) {
            ...
            throw new Error(`GitHub API responded with status ${csvUpdateResponse.status}`);
        }
